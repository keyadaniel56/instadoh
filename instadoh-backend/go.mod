...
require (
	...
	github.com/lightningnetwork/lnd v0.20.4-beta
...
// LND requires a fork of protobuf that supports hex-encoded bytes in JSON.
// These replacements mirror the ones in LND's own go.mod and are required
// because replace directives are not inherited by downstream consumers.
